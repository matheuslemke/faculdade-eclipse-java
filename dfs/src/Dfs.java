 ...
 * Equipe: faustao
 * 
 * Integrantes: Anderson, Lucas Didur e Lucas Padilha
 * 
...
public class Dfs
{
	...
	public static void dfsVisit(Vertice[] vertices, int vertice)
	{
		...
		for (Aresta adj : vertices[vertice].getAdj())
		{
			...
			if (vertices[adj.getIndice()].getCor().equals(Cor.BRANCO))
			{
	...
	public static void imprimir(Vertice[] vertices)
	{
		if (vertices != null)
			for (int i = 0; i < vertices.length; i++)
			{
				...
				if (vertices[i].getPai() == -1)
					System.out.println("NIL");
				else
					System.out
							.println(vertices[vertices[i].getPai()].getNome());
			}
	}
