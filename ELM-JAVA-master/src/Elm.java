...
public class Elm
{
	 ...
	 *            - Type of activation function: 'sig' for Sigmoidal function
	...
	public void loadMatrix()
	{
		...
		{
			int i = 0, j = 0;
			reader = new BufferedReader(new FileReader(new File("diabetes_train")));
			MatrixVectorReader mvr = new MatrixVectorReader(reader);
			// mvr.read
	...
	public void train(String filename) throws NotConvergedException
	{
		try
		{
			this.train_set = loadmatrix(filename);
		} catch (IOException e)
		{
			e.printStackTrace();
		}
	...
	private void train() throws NotConvergedException
	{
		...
		// for (int i = 0; i < InputWeight.numRows(); i++)
		// {
		// for (int j = 0; j < InputWeight.numColumns(); j++)
		// {
		// System.out.println(InputWeight.get(i, j) + " ");
		// }
		// System.out.println();
		// }
		...
		// DenseMatrix pinvHt = invers.getMPInverse(0.000001); //fast method,
		...
		// Classification," submitted to IEEE Transactions on Pattern Analysis
	...
	public void test(String TestingData_File)
	{
		...
		for (int i = 0; i < testY.numColumns(); i++)
		{
			System.out.print(i + ": ");
			for (int j = 0; j < testY.numRows(); j++)
			{
				System.out.print(testY.get(j, i) + " ~ " + testT.get(j, i) + "\t");
			}
			System.out.println();
		}
	}
